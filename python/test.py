...
# display formulas followed by a picture sequence: compare to its last picture
# (a long sequence wraps onto several polyeq rows)
for src,name in ((c1,"p1"),(c2,"p2")):
    for m in re.finditer(r'\$\$\n(.*?)\n\$\$', src, re.S):
        tex=m.group(1).strip()
        if 'A_' not in tex or '=' in tex or 'implies' in tex or r'\to' in tex or 'geq' in tex: continue
        row=re.search(r'((?:<div class="polyeq">.*?</div>\s*)+)', src[m.end():]).group(1)
